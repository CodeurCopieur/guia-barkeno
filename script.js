...
function matches(theme,idx,[es,fr]){
  const key=theme+"|"+idx;
  if(state.favOnly&&!favs.has(key))return false;
  if(state.q&&!(norm(es)+" "+norm(fr)).includes(state.q))return false;
  return true;
}

function speak(btn,text){
  if(!("speechSynthesis" in window)){btn.title="Audio non disponible";return}
...
function toggleFav(key,btn){
  ...
  saveFavs();
  btn.classList.toggle("on",favs.has(key));
}
...
function initNav(){
  const nav=$("#chips");
  const sync=()=>nav.querySelectorAll(".chip").forEach((b,i)=>b.setAttribute("aria-pressed",CHIPS[i].is()));
  CHIPS.forEach(c=>{
...
function render(){
  ...
  THEMES.forEach(theme=>{
    ...
    theme.phrases.forEach((p,idx)=>{
      if(matches(theme.id,idx,p)){
        const key=theme.id+"|"+idx;
        rows.push(`<div class="card">
          <div class="top">
            <div class="es">${hi(p[0],state.q)}</div>
            <div class="actions">
              <button class="btn say" data-say="${esc(p[0])}" aria-label="Écouter">${ICON_SPEAK}</button>
              <button class="btn fav${favs.has(key)?" on":""}" data-key="${key}" aria-label="Favori">${ICON_STAR}</button>
            </div>
          </div>
          <div class="fr">${hi(p[1],state.q)}</div>
        </div>`);
        total++;
      }
    });
  ...
  $("#count").textContent=`${total} phrase${total>1?"s":""}`;
  document.querySelectorAll(".say").forEach(b=>b.onclick=()=>speak(b,b.dataset.say));
...
const si=$("#search"),ci=$("#clear");
si.addEventListener("input",()=>{
  state.q=si.value.trim();
  ci.style.display=si.value?"block":"none";
  render();
});
